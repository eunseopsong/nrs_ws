...
YMatrix MotionBlender6D::blendMotion(double Defualt_travelTime)
{
    /*** Error Handling ***/
    if(position.rows() < 2)
    {
    ...
    if(ang_velocity.size() != position.rows())
    {
        std::cerr << "Error: Angular velocity vector size must match the number of positions." << std::endl;
        std::exit(EXIT_FAILURE);
    }
    if(holding_time.size() != position.rows())
    {
        std::cerr << "Error: Holding-time vector size must match the number of positions." << std::endl;
        std::exit(EXIT_FAILURE);
    }
