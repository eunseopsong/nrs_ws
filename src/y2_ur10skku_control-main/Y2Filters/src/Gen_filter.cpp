...
float Y_KalmanFilter::KalmanFilter(float input)
{
...
float Y_KalmanFilter::KalmanFilter1D(float input)
{
...
Y_MovFilter::Y_MovFilter(int mv_num_)
: mv_num(mv_num_), running_sum(0.0f) {
    saved_data.reserve(mv_num);  // Pre-allocate to avoid reallocations
}

float Y_MovFilter::MovFilter(float input)
{    
    if(mv_num > counter)
    {   
        // Still filling the buffer
        saved_data.push_back(input);
        running_sum += input;  // Add to running sum
        counter++;
        output = 0.0f;  // No output yet until buffer is full
    }
    else
    {
        // Buffer is full, do circular buffer update
        // Remove oldest value (at position counter % mv_num)
        int oldest_idx = counter % mv_num;
        running_sum -= saved_data[oldest_idx];
        
        // Add new value
        running_sum += input;
        saved_data[oldest_idx] = input;
        
        // Calculate output from running sum (O(1) instead of O(n))
        output = running_sum / (float)mv_num;
        
        counter++;  // Move to next position for circular buffer
    }

    return output;
}

Y_FreqFilter::Y_FreqFilter(double Ts_)
:Ts(Ts_) {}

float Y_FreqFilter::HPF(float input)
{
    double HPF_out = 0;
...
float Y_FreqFilter::LPF(float input)
{
    double a1,b0,b1,w0;
    double LPF_out;
...
float Y_FreqFilter::BSF(float input)
{
    double BSF_out = 0;
