...
public:
  TxtTrajectoryExecutor() : Node("txt_trajectory_executor")
  {
    pub_ = this->create_publisher<sensor_msgs::msg::JointState>(
      "/isaac_joint_commands", rclcpp::QoS(10).best_effort());
    ...
    timer_ = this->create_wall_timer(
      std::chrono::milliseconds(2), std::bind(&TxtTrajectoryExecutor::publish_next_point, this));
  }
...
private:
  ...
  void publish_next_point()
  {
    ...
    double elapsed_ms = std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(
      t_now - start_time_).count();
    double delta_ms = std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(
      t_now - last_time_).count();
    last_time_ = t_now;
