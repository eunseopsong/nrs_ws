...
def normalize_data(input_data, target_data):
    ...
    return input_data_norm, target_data_norm, input_min, input_max, target_min, target_max, denormalize
# %% Main Entry Point for ROS 2
def main():
    print("✅ TWC Inference Node 시작")
    input_data, _, _, target_data = load_and_preprocess_data(data_num=2)
    input_data_norm, target_data_norm, _, _, _, _, _ = normalize_data(input_data, target_data)

    print(" - input_data_norm shape:", input_data_norm.shape)
    print(" - target_data_norm shape:", target_data_norm.shape)
